...
output_repo = git.Repo.init(commons.output_pot_dir)
...
for key in commons.git_repos:
    print("Cloning {}".format(key))
    local_dir = commons.git_repos[key]['path']
    repo = commons.git_repos[key]['url']
    if not os.path.exists(local_dir):
        git.Git().clone(repo, local_dir)
    repo_obj = git.Repo(local_dir)

    commons.git_repos[key]['last_commit'] = repo_obj.git.log("-n1","--pretty=oneline").split(' ')[0]
...
templatedata_dict = {}
template_files_prefix = "{}/struct-wikipages/en/Template:".format(commons.git_repos["WikiToLearn"]['path'])
...
with open("{}templatedata_dict.json".format(commons.tmp_dir), 'w') as outfile:
    json.dump(templatedata_dict, outfile)
    outfile.close()

    pot_lib_repo_key = "WikiToLearn"
    output_pot_file = "{}templatedata_dict.pot".format(commons.output_pot_dir)
    project_id_version = "{} {}".format(pot_lib_repo_key,commons.git_repos[pot_lib_repo_key]['last_commit'])
    gen_pot = True
    ...
    if gen_pot:
        cmd = ["json2po","-P","{}templatedata_dict.json".format(commons.tmp_dir),output_pot_file]
        call(cmd)
        pot = polib.pofile(output_pot_file)
...
for input_json_file in glob.glob("{}/*/i18n/en.json".format(commons.tmp_git_dir)):
    pot_lib_repo_key = None
    for repo_check_key in commons.git_repos:
        if input_json_file.startswith(commons.git_repos[repo_check_key]['path']):
            pot_lib_repo_key = repo_check_key
    output_pot_file = "{}{}.pot".format(commons.output_pot_dir,input_json_file[len(commons.tmp_git_dir):-len("/i18n/en.json")])
    project_id_version = "{} {}".format(pot_lib_repo_key,commons.git_repos[pot_lib_repo_key]['last_commit'])
    gen_pot = True
    if os.path.exists(output_pot_file):
        pot = polib.pofile(output_pot_file)
...
with open("{}pages_id.json".format(commons.tmp_dir), 'w') as outfile:
    ...
    if gen_pot:
        cmd = ["json2po","-P","{}pages_id.json".format(commons.tmp_dir),output_pot_file]
        call(cmd)
...
for mw_page_title in mw_pages_files:
    ...
    pot_lib_repo_key = "WikiPages-en"
    output_pot_file = "{}{}.pot".format(commons.output_pot_dir,placeholder_dict[mw_page_title])
    project_id_version = "{} {}".format(pot_lib_repo_key,commons.git_repos[pot_lib_repo_key]['last_commit'])
    gen_pot = True
    if os.path.exists(output_pot_file):
        pot = polib.pofile(output_pot_file)
